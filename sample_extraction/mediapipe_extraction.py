...
from sample_extraction.utils import display_annotated_image, get_bbox_from_kps, get_data_path, get_average_keypoint, associate_bboxes_to_annotations, load_fairset_annotations
...
def mediapipe_results_to_2d_keypoints(
        mp_landmarks: List[NormalizedLandmark],
        max_width: int, max_height: int) -> List[Keypoint]:
    keypoints: List[Keypoint] = []
    for i, landmark in enumerate(mp_landmarks):
        keypoints.append(Keypoint(id=i, x=int(
            max_width*landmark.x), y=int(max_height*landmark.y)))
    return keypoints
...
def estimate(frame: np.ndarray, landmarker):
    ...
    for mp_kps in mp_result.face_landmarks:
        kps = mediapipe_results_to_2d_keypoints(
            mp_kps, frame.shape[1], frame.shape[0])
    ...
    for i in range(len(images)):
        ...
        for row, col in association_indices:
            ...
            for custom_idx, mp_indices in KEYPOINT_MAPPING.items():
                if len(mp_indices):
                    kps.append(get_average_keypoint(
                        [kp for kp in estimations[row][1] if kp.id in mp_indices], custom_idx))

            association_accepted = ious[row][col] > MEDIAPIPE.get("min_iou", 0.4)
            if not association_accepted:
                display_image = list(images_annotations.values())[col]["bbox"].annotate_image(
                    np.array(im), (0, 255, 0))
                key = display_annotated_image(
                    display_image, kps, estimations[row][0])
                if key == ord("a"):
